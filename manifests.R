...
# Load data
# ------------------
trial_dat_raw <- read_csv("data/data_atc_all.csv", show_col_types = FALSE)
slider_dat_raw <- read_csv("data/data_atc_sliders_all.csv", show_col_types = FALSE)
postblock_dat_raw <- read_csv("data/data_atc_postblock_all.csv", show_col_types = FALSE)
...
p_acc_block <- ggplot() +
  ...
  labs(
    x = "Block",
    y = "Accuracy (%)",
    title = "Accuracy by block (ATC task)",
    subtitle = MOREY_SE_SUBTITLE
  ) +
...
p_calib_manual_acc <- ggplot(
  ...
  labs(
    ...
    title = "Calibration vs Manual accuracy (ATC task)"
  ) +
