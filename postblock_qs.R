...
# Load data
# ------------------
dat <- read_csv("data/data_atc_postblock_all.csv")
str(dat)
...
# Load slider data
# ------------------
dat_slider_raw <- read_csv("data/data_atc_sliders_all.csv")
str(dat_slider_raw)
...
# If there is ever more than one row per participant/block/slider_key,
# average within participant first
subj_slider_summary <- dat_slider %>%
  group_by(participant_id, block, slider_key) %>%
  summarise(
    response = mean(response, na.rm = TRUE),
    .groups = "drop"
  )
...
slider_plot_dat <- get_morey_cf(
  ...
  subject_col = "participant_id",
  within_cols = c("block", "slider_key"),
  dv_col = "response"
)
